...
const PublicTeamView = () => {
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'radial-gradient(circle at 50% 0%, #1e1b4b 0%, #020617 100%)', color: 'white', padding: '2rem', fontFamily: 'Outfit, sans-serif' }}>
            {/* Header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                    {config.tournament_logo && (
                        <img src={`http://localhost:5000${config.tournament_logo}`} style={{ height: 70, background: 'white', padding: '8px', borderRadius: '12px' }} alt="Tournament" />
                    )}
                    <div>
                        <h1 style={{ margin: 0, fontSize: '2.5rem', fontWeight: '900' }}>Team Squad</h1>
                        <p style={{ margin: '5px 0 0 0', opacity: 0.6 }}>Public View - Read Only</p>
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                    ...
                    >
                        <QrCodeIcon size={18} /> {showQR ? 'Hide' : 'Show'} QR Code
                    </button>
                    <button
            ...
            <div ref={squadRef} style={{
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '25px' }}>
                        ...
                        <div>
                            <h2 style={{ margin: 0, fontSize: '3rem', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '2px' }}>{team.name}</h2>
                    ...
                    <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: '3rem', fontWeight: '900', color: '#ffd700' }}>{totalPlayers}</div>
                        <div style={{ fontSize: '0.9rem', opacity: 0.7 }}>Total Players</div>
                        <div style={{ marginTop: '15px', fontSize: '1.2rem', fontWeight: 'bold', color: '#22c55e' }}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0 10px' }}>
                        <thead>
                            <tr style={{ background: 'rgba(255,255,255,0.05)' }}>
                                <th style={{ padding: '15px', textAlign: 'left', borderRadius: '10px 0 0 10px' }}>Player</th>
                                <th style={{ padding: '15px', textAlign: 'left' }}>Category</th>
                                <th style={{ padding: '15px', textAlign: 'left' }}>Set</th>
                                <th style={{ padding: '15px', textAlign: 'right', borderRadius: '0 10px 10px 0' }}>Price</th>
                            </tr>
                        </thead>
                        <tbody>
                            {team.owner_name && (
                                <tr style={{ background: 'rgba(236, 72, 153, 0.1)', borderRadius: '10px' }}>
                                    <td style={{ padding: '15px', borderRadius: '10px 0 0 10px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                            <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(236, 72, 153, 0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                <Shield size={20} color="#ec4899" />
                                            </div>
                                            <div>
                                                <div style={{ fontWeight: 'bold', color: '#ec4899' }}>{team.owner_name}</div>
                                                <span style={{ fontSize: '0.7rem', color: '#ec4899' }}>★ OWNER</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td style={{ padding: '15px' }}>Sponsor</td>
                                    <td style={{ padding: '15px', opacity: 0.6 }}>Reserved</td>
                                    <td style={{ padding: '15px', textAlign: 'right', fontWeight: 'bold', borderRadius: '0 10px 10px 0' }}>₹ 0</td>
                                </tr>
                            )}
                            {team.captain_name && (
                                <tr style={{ background: 'rgba(34, 197, 94, 0.1)', borderRadius: '10px' }}>
                                    <td style={{ padding: '15px', borderRadius: '10px 0 0 10px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                            <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(34, 197, 94, 0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                <Award size={20} color="#22c55e" />
                                            </div>
                                            <div>
                                                <div style={{ fontWeight: 'bold', color: '#22c55e' }}>{team.captain_name}</div>
                                                <span style={{ fontSize: '0.7rem', color: '#22c55e' }}>© CAPTAIN</span>
                                            </div>
                                        </div>
                                    </td>
                            ...
                            {squad.map((player, idx) => (
                                <tr key={player.id} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '10px' }}>
                                    <td style={{ padding: '15px', borderRadius: '10px 0 0 10px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                            <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#334155', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                {player.image ? (
                                            ...
                                            <div>
                                                ...
                                                {player.is_icon === 1 && <span style={{ fontSize: '0.7rem', color: '#ffd700' }}>★ ICON</span>}
                                            </div>
                                        </div>
                                    </td>
                                    <td style={{ padding: '15px' }}>{player.category}</td>
                                    <td style={{ padding: '15px', opacity: 0.6 }}>{player.auction_set}</td>
                                    <td style={{ padding: '15px', textAlign: 'right', fontWeight: 'bold', color: '#ffd700', borderRadius: '0 10px 10px 0' }}>{formatMoney(player.sold_price)}</td>
                                </tr>
                            ))}
                            {totalPlayers === 0 && (
                                <tr>
                                    <td colSpan="4" style={{ padding: '60px', textAlign: 'center', opacity: 0.3 }}>
                                        <Shield size={60} style={{ margin: '0 auto 20px auto', opacity: 0.2 }} />
                ...
                <div style={{
                    ...
                    <div>
                        <div style={{ fontSize: '0.8rem', opacity: 0.6 }}>Total Spent</div>
                        <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#ef4444' }}>{formatMoney(spent)}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: '0.8rem', opacity: 0.6 }}>Budget Utilization</div>
            ...
            {/* Watermark */}
            <div style={{ textAlign: 'center', marginTop: '30px', opacity: 0.4, fontSize: '0.9rem' }}>
