...
const Display = () => {
    ...
    useEffect(() => {
        if (auction.status === 'lucky_dip') {
            ...
            axios.get(getApiUrl('/api/players')).then(res => {
            ...
            return (
                <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(circle at center, #064e3b 0%, #061713 100%)', position: 'relative' }}>
                    ...
                    <div style={{ position: 'absolute', top: 40, width: '100%', display: 'flex', justifyContent: 'space-between', padding: '0 50px', alignItems: 'center' }}>
                        {config?.tournament_logo && <img src={getImageUrl(config.tournament_logo)} style={{ height: 100, background: 'white', padding: '10px', borderRadius: '15px' }} />}
                        {mainSponsor && <img src={getImageUrl(mainSponsor.logo)} style={{ height: 80, background: 'white', padding: '10px', borderRadius: '15px' }} />}
                    </div>
            ...
            <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(circle at center, #1e293b 0%, #0f172a 100%)', overflow: 'hidden' }}>
                <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 5 }}>
                    {config?.tournament_logo ? (
                        <img src={getImageUrl(config.tournament_logo)} style={{ height: 200, filter: 'drop-shadow(0 0 20px gold)', background: 'white', padding: '15px', borderRadius: '20px' }} />
                    ) : (
                ...
                {mainSponsor && (
                    <div style={{ position: 'absolute', bottom: 120, right: 40, display: 'flex', alignItems: 'center', gap: '15px' }}>
                        ...
                        <img src={getImageUrl(mainSponsor.logo)} style={{ height: 60, background: 'white', padding: '5px', borderRadius: '10px' }} />
                    </div>
                )}
                <MarqueeFooter />
                <style>{`
    ...
    return (
        <div style={{ height: '100vh', overflow: 'hidden', padding: '2rem', background: '#0f172a', fontFamily: 'Outfit, sans-serif' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #333', paddingBottom: '1rem', position: 'relative' }}>
                {config?.tournament_logo && (
                    <img src={getImageUrl(config.tournament_logo)} style={{ height: 60, marginRight: '20px', background: 'white', padding: '5px', borderRadius: '8px' }} />
                )}
                ...
                {config?.sponsor_logo && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginRight: '20px' }}>
                        ...
                        <img src={getImageUrl(config.sponsor_logo)} style={{ height: 40, background: 'white', padding: '3px', borderRadius: '6px' }} />
                    </div>
                )}
                    ...
                    <div style={{ marginTop: '2rem', minHeight: '150px' }}>
                        <AnimatePresence mode="wait">
                            {lastBidderTeam ? (
                                ...
                                >
                                    {lastBidderTeam.logo && <img src={getImageUrl(lastBidderTeam.logo)} style={{ height: 100, background: 'white', padding: '8px', borderRadius: '15px' }} />}
                                    <div>
